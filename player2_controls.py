...
class P2Controls:
    def __init__(self, character):
        ...
        self.logo = pygame.image.load('assets/graphics/p2.png')
        self.hp = 100  # Health of the Character
        self.alive = True  # Check if the Character is Alive
        self.rectStand = pygame.Rect((800, 280, 100, 300))  # Hit box Standing
    ...
    def draw_player1(self, surface):
        surface.blit(pygame.transform.scale(self.logo, (406 * 0.5, 448 * 0.5)),
                     (self.rectStand.x - 50, self.rectStand.y - 190))
        pygame.draw.rect(surface, (0, 0, 255), self.rectStand)
    ...
    def movement_player1(self, screen_width, screen_height):
        ...
        if self.key[pygame.K_LEFT]:
            ...
            self.dx -= self.SPEED
            self.movements = 1
            print("2 Walking Left")
        if self.key[pygame.K_RIGHT]:
            self.blocking = False
            self.dx = self.SPEED
            self.movements = 2
            print("2 Walking Right")
        if self.key[pygame.K_UP] and self.jumping == False:
            self.blocking = False
            self.jumping = True
            self.vel_y = -35
            self.movements = 3
